...
import com.accenture.customers.dto.*;
...
//    @GetMapping(value = "/buld-info",produces = MediaType.APPLICATION_JSON_VALUE)
//    public ResponseEntity<ResponseDto> getBuildInfo(){
//        return ResponseEntity
//                .status(HttpStatus.OK)
//                .body(new ResponseDto("202", buildVersion));
//    }
    ...
    @GetMapping(value = "/fetchAccountById/{customerId}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<CustomerDto> fetchAccountById(@PathVariable Long customerId){
        CustomerDto customerDto = iCustomerService.findById(customerId);
